...
- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송
...
- Data
  ...
  - 4번째 res_code : 1byte. Response code.(00 : 정상, 01: 비정상(invalid))
- **Packet Structure**
...
- 앱 \<\<- 인슐린펌프: BT_START_REQ(0x01) 메시지 전송 (BLE통신 시작 요청)

- 앱 -\>\> 인슐린펌프: BT_CONNECTABLE_CTRL_REQ(0x02) 메시지 전송 (BLE통신 시작 요청)
...
- 앱 -\>\> 인슐린펌프: BT_STATE_REQ(0x04) 메시지 전송 (장치 상태 값 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code에 장치 상태 값 응답)
  
- 앱 \<\<- 인슐린펌프: BT_STATE_IND(0x05) 메시지 전송 (상태 응답)

1.  상태 알림 Flow

- 앱 \<\<- 인슐린펌프: BT_STATE_IND 메시지 전송 (상태가 변경될 때 자동 알림)
...
- 앱 -\>\> 인슐린펌프: BT_CUR_TIME_IND(0x06) 메시지 전송 (현재 시간 동기화)

- 앱 \<\<- 인슐린펌프: BT_CUR_TIME_RES(0x44) 메시지 전송 (현재 시간 동기화)

2.  배터리 잔량정보 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_BATT_DATA_REQ(0x70) 메시지 전송 (배터리 잔량정보 요청)

- 앱 \<\<- 인슐린펌프: BT_BATT_DATA_RES(0x72) 메시지 전송 (배터리 잔량정보 응답)

3.	배터리 부족일 경우 자동 전송 Flow (펌프로 부터 패킷이 수신되면 자동 업데이트)

- 앱 \<\<- 인슐린펌프: BT_BATT_DATA_IND(0x71) (배터리 level변화시 자동 전송)

4.  PID 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_PUMP_PID_REQ(0x09) 메시지 전송 (인슐린펌프 PID 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_PUMP_PID_RES(0x0A) 메시지 전송 (인슐린펌프 PID 응답)

5.  Firmware version 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프 : BT_PUMP_FW_REQ(0x3E) 메시지 전송 (Firmware version 요청)

- 앱 \<\<- 인슐린펌프 : BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프 : BT_PUMP_FW_RES(0x3F) 메시지 전송 (Firmware version 응답)
...
- 앱 -\>\> 인슐린펌프: BT_LOG_REQ(0x1D) 메시지 전송 (인슐린펌프에 저장된 이력 데이터 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_DATA_START_IND(0x0B) 메시지 전송 (대량 데이터 시작 알림)

- 앱 \<\<- 인슐린펌프: 저장된 이력 Data 전송(헤더 없음, 최대 15일 기준)

- 앱 \<\<- 인슐린펌프: BT_DATA_END_IND(0x0C) 메시지 전송 (대량 데이터 종료 알림)
...
- 앱 -\>\> 인슐린펌프: BT_STOP_CTRL_REQ(0x0D) (일시 정지 또는 해제 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x0) 메시지 전송 (res_code 정상 0x00 전송)
...
- 앱 -\>\> 인슐린펌프: BT_TIME_BASE_SET_REQ(0x0F) (기초 값 설정 요청, 6회 전송)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_SET_RES(0x12) (설정 요청에 대한 답변)

2.  식사 값 설정 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_MEAL_SET_REQ(0x10) (식사 값 설정 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_SET_RES(0x12) (설정 요청에 답변)
...
- 앱 -\>\> 인슐린펌프: BT_EXERCISE_SET_REQ(0x13) 메시지 전송 (운동모드 설정 및 주입 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (운동모드 진행중이면 res_code invalid 0x01 전송)

- 앱 \<\<- 인슐린펌프: BT_EXERCISE_INJ_START_IND(0x29) 메시지 전송 (운동모드 시작 알림)

- 앱 \<\<- 인슐린펌프: BT_EXERCISE_INJ_STOP_IND(0x2A) 메시지 전송 (운동모드 종료 알림)

2.  회식모드 설정 및 주입 요청, 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_RECEPTION_SET_REQ(0x14) 메시지 전송 (회식모드 설정 및 주입 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (회식모드 진행중이면 res_code invalid 0x01 전송)

- 앱 \<\<- 인슐린펌프: BT_RECEPTION_INJ_START_IND(0x2B) 메시지 전송 (회식모드 시작 알림)

- 앱 \<\<- 인슐린펌프: BT_RECEPTION_INJ_STOP_IND(0x2C) 메시지 전송 (회식모드 종료 알림)

3.  주입정보 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_INJ_INFO_REQ(0x15) 메시지 전송 (주입정보 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_INJ_INFO_RES(0x16) 메시지 전송 (주입정보 응답)

4.  주입 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_INJ_REQ(0x17) 메시지 전송 (주입 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_INJ_START_IND(0x3A) 메시지 전송 (주입 시작 알림)

- 앱 \<\<- 인슐린펌프: BT_INJ_STOP_IND(0x3B) 메시지 전송 (주입 종료 알림)

5.  강제 정지 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프 : BT_INJ_STOP_REQ(0x37) 메시지 전송 (강제 정지 요청)
...
- 앱 \<\<- 인슐린펌프 : BT_ERR_IND(0x19) 메시지 전송 (경고, 알람 등 이슈사항 알림)
...
- 앱 -\>\> 인슐린펌프 : BT_LOG_INJ_QNT_REQ(0x1E) 메시지 전송 (금일 누적량 이력 정보 요청)

- 앱 \<\<- 인슐린펌프 : BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프 : BT_LOG_INJ_QNT_IND(0x1F) 메시지 전송 (금일 누적량 이력 정보 전달)
...
- 앱 -\>\> 인슐린펌프: BT_EAT_VALUE_REQ(0x2D) 메시지 전송 (식사 설정 값 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_EAT_VALUE_RES(0x2E) 메시지 전송 (식사 설정 값 전달)

2.  기초 설정 값 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_BASE_VALUE_REQ(0x2F) 메시지 전송 (기초 설정 값 요청)

- 앱 \<\<- 인슐린펌프: BT_MSG_RES(0x00) 메시지 전송 (res_code 정상 0x00 전송)

- 앱 \<\<- 인슐린펌프: BT_BASE_VALUE_RES(0x30) 메시지 전송 (기초 설정 값 전달)
...
- 앱 \<\<- 인슐린펌프: BT_BASE_VALUE_IND(0x40) 메시지 전송 (기초 설정 값 변경 시 전달)

2.  식사 설정 값 알림 Flow

- 앱 \<\<- 인슐린펌프: BT_LOG_INJ_SET_1_IND(0x20) 메시지 전송 (식사 설정 값 변경 시 전달)
...
- 앱 -\>\> 인슐린펌프: BT_PRS_APP_PASSWD_IND(0x3C) 메시지 전송 (기존 Password 정보 전달)

2.  신규 패스워드 전송 Flow

- 앱 \<\<- 인슐린펌프: BT_NEW_APP_PASSWD_IND(0x3D) 메시지 전송 (신규 Password 정보 전달)

3.  패스워드 요청 및 응답 Flow

- 앱 -\>\> 인슐린펌프: BT_PRS_APP_PASSWD_REQ(0x41) 메시지 전송 (현재 Password 조회 요청)

- 앱 \<\<- 인슐린펌프: BT_PRS_APP_PASSWD_RES(0x42) 메시지 전송 (현재 Password 전달)
...
4.  BT_NEW_APP_PASSWD_RES(0x42) (인슐린펌프 -\> 앱)
